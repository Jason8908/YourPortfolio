  ...
  return fetch(
    `https://ca.indeed.com/viewjob?jk=${encodeURIComponent(id)}&spa=1`,
    {
      headers: {
        ...
        Referer:
          "https://ca.indeed.com/jobs?q=test+engineer&l=Scarborough%2C+ON&from=searchOnHP&vjk=0030a5c1d61c3f8a",
  ...
  return Promise.allSettled(
    ids.map(
      (id, ind) =>
        new Promise((res) => {
          setTimeout(
            () => getIndeedJobFromId({ id }).then((out) => res(out)),
            REQUEST_DELAY * ind,
          );
        }),
    ),
  ...
  try {
    ...
    const employer =
      jsonResponse.body.jobInfoWrapperModel.jobInfoModel.jobInfoHeaderModel
        .companyName;
    const benefits = jsonResponse.body.benefitsModel?.benefits?.map(
    ...
    const attributes = jsonResponse.body.mosasicData.provider[
      "js-match-insights-provider"
    ].initialData.attributes.map((a) => a.label);
