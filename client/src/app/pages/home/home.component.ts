...
export class HomeComponent {
  ...
  constructor(private authService: SocialAuthService, private router: Router, private route: ActivatedRoute, private apiService: ApiService, private cookieService: CookieService, private localStorage: LocalStorageService) {
    ...
    this.authService.authState.subscribe((user) => {
      ...
      this.authService.getAccessToken(GoogleLoginProvider.PROVIDER_ID).then(token => {
        ...
        this.apiService.sendAuth(token, user.firstName, user.lastName, user.email).subscribe((response) => {
          ...
          this.cookieService.set(CookieLabels.AUTH_TOKEN, response.data.accessToken);
